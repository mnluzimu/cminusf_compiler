...
class GVN : public Pass {
  public:
    ...
    std::shared_ptr<GVNExpression::PhiExpression> valuePhiFunc(BasicBlock *bb_ptr, std::shared_ptr<GVNExpression::Expression>,
                                                               const partitions &);
    std::shared_ptr<GVNExpression::Expression> valueExpr(Value *instr);
    ...
    //my functions
    bool dfs_bb(BasicBlock * bb_ptr, bool is_first);
  ...
  public:
    ...
    //my variables
    partitions Top{nullptr}; //Top is a partition with only a nullptr in it
    std::set<BasicBlock*> visited; //to mark if a bb has been visited
