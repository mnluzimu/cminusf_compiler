...
class Expression {
  public:
    ...
    enum gvn_expr_t { e_constant, e_bin, e_phi, e_cmp, e_fcmp, e_alloca, e_load, e_call, e_gep, e_si2fp, e_fp2si, e_zext, e_argu, e_global, e_index };
...
class ArguExpression : public Expression {
  public:
    ...
    virtual std::string print() { return argu->print(); }
...
class LoadExpression : public Expression {
  public:
    static std::shared_ptr<LoadExpression> create(std::shared_ptr<Expression> alloca_ptr) { return std::make_shared<LoadExpression>(alloca_ptr); }
    virtual std::string print() { std::string str = "load"; return str; }
    // we leverage the fact that alloca instrs in lightIR have unique addresses (hopfully)
    bool equiv(const LoadExpression *other) const { return *(alloca_ptr) == *(other->alloca_ptr); }
    LoadExpression(std::shared_ptr<Expression> alloca_ptr) : Expression(e_load), alloca_ptr(alloca_ptr) {}

  private:
    std::shared_ptr<Expression> alloca_ptr;
};
...
class ZextExpression : public Expression {
  public:
    ...
    bool equiv(const ZextExpression *other) const { 
      printf("enter equiv zext\n"); 
      std::cout << orig->print() << std::endl;
      std::cout << other->orig->print() << std::endl;
      if (orig == other->orig) {
        printf("orig equal\n");
      }
      if (dest_ty_ == other->dest_ty_) {
        printf("dest type equal\n");
      }
...
class GVN : public Pass {
  public:
    ...
    std::shared_ptr<GVNExpression::PhiExpression> valuePhiFunc(BasicBlock *bb_ptr, std::shared_ptr<GVNExpression::Expression>,
                                                               const partitions &);
    std::shared_ptr<GVNExpression::Expression> valueExpr(Value *instr, std::unique_ptr<FuncInfo> &func_info_, partitions& p);
    std::shared_ptr<GVNExpression::Expression> getVN(const partitions &pout,
                                                     std::shared_ptr<GVNExpression::Expression> ve);

    std::shared_ptr<GVNExpression::Expression> get_index_expression (int index, const partitions &P);
